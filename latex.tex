...
\begin{itemize}
	\item \textbf{pdflatex} erstellt die PDF Datei (z.B. unter Windows pdflatex.exe) mit den Argumenten \verb|-interaction=nonstopmode "\%wm" |
	\item \textbf{biber} erstellt das Literatur-Verzeichnis (z.B. unter Windows biber.exe) mit den Argumenten \verb|"\%wm" |
	\item \textbf{makeindex} erstellt das Inhaltsverzeichnis und die Nummerierung der Kapitel,... (z.B. unter Windows makeindex.exe) mit den Argumenten\\ \verb|"\%tm.idx" -t "\%tm.ilg" -o "\%tm.ind" |
	\item \textbf{makeglossaries} erstellt die Glossaren (z.B. unter Windows makeglossaries-lite.exe) mit den Argumenten \verb|-s "%tm.ist" -t "%tm.glg" -o "%tm.gls" "%tm.glo"|
\end{itemize}
...
\subsubsection{Online-\latex-Editoren}

Es gibt mehrere Webseiten die Online-\latex-Editoren anbieten. Dazu gehören:
\begin{itemize}
	\item Overleaf\footnote{\url{https://de.overleaf.com/}}
	\item Tex Page\footnote{\url{https://www.texpage.com/}}
	\item crixet\footnote{\url{https://crixet.com/}}
\end{itemize}
Oft sind auf diesen Webseiten jedoch Begrenzungen beim Kompilieren aktiv. In diesem Fall können z.B. eigene Seiten darauf erstellt werden, das gesammelte Dokument der Gruppe sollte jedoch auf einem eigenen Computer erstellt werden. Weiters kann es sein, dass das von den Webseiten erstellte Ergebnis nicht korrekt ist im Bezug auf Verweise, Literatur und Ähnliches, da einige der speziellen Einstellungen der Vorlage eventuell nicht funktionieren.
...
%
\begin{itemize}
\item \emph{\citetitle{Schmidt01}}\cite{Schmidt01}
\item \emph{\citetitle{Oetiker01}}\cite{Oetiker01}
\end{itemize}
%
\noindent
...
%
\begin{itemize}
  \item \emph{\citetitle{Kopka98}}\cite{Kopka98}
\end{itemize}
%
...
\begin{quote}
  ...
  \texttt{subcaption} \cite{Cochran95}.
\end{quote}
