...
\subsection{{\tt citetitle} und {\tt cite}}

Für Literaturverweise auf den Titel von Quellen im laufenden Text verwendet man in \latex die Anweisung
\begin{itemize}
...
\noindent%
Zu jedem Verweis muss außerdem noch ein Verweis auf die ausführlichen Informationen zu der Quelle im Literaturverzeichnis angegeben werden.

\begin{itemize}
\item[] \verb!\cite{!\textit{Verweise}\verb!}! oder
\item[] \verb!\cite[!\textit{Zusatztext}\verb!]{!\textit{Verweise}\verb!}! oder
\item[] \verb!\cite[!\textit{Vorwort}\verb!][!\textit{Zusatztext}\verb!]{!\textit{Verweise}\verb!}!.
\end{itemize}
...
\begin{itemize}
\item[] \verb!Mehr dazu findet sich in \citetitle{Kopka98}\cite[][]{Kopka98}.! \newline
      $\rightarrow$ Mehr dazu findet sich in \citetitle{Kopka98}\cite[][]{Kopka98}.
\item[] \verb!Mehr über \emph{Styles} in \citetitle{Kopka98}\cite[][Kap.\ 3]{Kopka98}.! \newline
      $\rightarrow$ Mehr über \emph{Styles} in \citetitle{Kopka98}\cite[][Kap.\ 3]{Kopka98}.
\item[] \verb!Die Angaben in \citetitle{Ears99}\cite[][S.\ 274--277]{Ears99} sind falsch.! \newline
      $\rightarrow$ Die Angaben in \citetitle{Ears99}\cite[][S.\ 274--277]{Ears99} sind aus meiner Sicht falsch.
\item[] \verb!überholt sind auch \citetitle{Ears99,Microsoft01,Duden97}!\newline\verb!\cite[][]{Ears99,Microsoft01,Duden97}.! \newline
      $\rightarrow$ überholt sind auch \citetitle{Ears99,Microsoft01,Duden97}\cite[][]{Ears99,Microsoft01,Duden97}.
\end{itemize}
...
hier. \cite[siehe][]{Oetiker01} $\leftarrow$~FALSCH!

\subsubsection{Verweis nicht sichtbar}
Wenn eine Quelle\cite{Kopka98} mehrfach in einem Kapitel verwendet wird, müssen alle Stellen gekennzeichnet werden, an denen die Quelle\cite{Kopka98} verwendet wird. Ein einfacher Verweis auf die Quelle\cite{Kopka98} am Anfang des Kapitels ist nicht ausreichend.
...
Möglichkeiten:
\begin{enumerate}
\item Das Literaturverzeichnis manuell zu formatieren \cite[siehe][S.\ 56--57]{Kopka98}.
\item Die Verwendung von BibTeX und einer zugehörigen Literaturdatenbank
\cite[siehe][S.\ 245--255]{Kopka98}.
\end{enumerate}
...
\citetitle{Taylor96,Patashnik88}\cite{Taylor96,Patashnik88}.
...
\begin{itemize}
\item Buch (\texttt{@book})\cite{BurgerBurge06}
\item Mehrbändige Bücher (\texttt{@mvbook})
\item Teil eines Buches (\texttt{@inbook})
\item Buchähnliche Publikation (\texttt{@booklet}) 
\item Buchbeitrag, Beitrag in einem Sammelband (\texttt{@incollection})\cite{Ears99}
\item Konferenzbeitrag, Beitrag in einem Tagungsband (\texttt{@inproceedings})\cite{Burger87}
\item Zeitschriftenbeitrag, Journal Paper (\texttt{@article})\cite{Guttman01}
\item Dissertation (\texttt{@phdthesis})\cite{Eberl87}
\item Diplomarbeit (Universität, \texttt{@mastersthesis})\cite{Wintersberger00}
\item Bachelorarbeit (\texttt{@masterthesis} modifiziert)\cite{Bacher04}
\item Bericht, Technical Report (\texttt{@techreport})\cite{Beeler48}
\item Handbuch, Manual, Online-Dokumentation (\texttt{@manual})\cite{Microsoft01}
\item Sonderfälle wie \zB Patente\cite{PAT07}, Normen\cite{DIN9241-11}\cite{DIN9241-110}, Audio-CDs\cite{Zappa95}, Filme\cite{Nosferatu}, Persönliche Kommunikation\cite{Kreisky75} (\texttt{@misc})
...
Ein Intervall über \emph{mehrere} Monate kann man in der (etwas eigenartigen) BibTeX-Syntax so angeben (verwendet \zB\ für "`Mai/Juni"' in \citetitle{Guttman01}\cite{Guttman01}):
\begin{quote}
\verb!month=MAY # "/" # JUN!
\end{quote}
